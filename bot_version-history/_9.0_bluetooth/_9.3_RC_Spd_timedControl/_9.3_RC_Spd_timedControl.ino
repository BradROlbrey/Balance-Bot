...
const int props = BLEWriteWithoutResponse | 0;
// maybe when the transmitter is waiting for a response, the bot
// is trying to send it out, but keeps getting interrupted by the encoders

// Services and Characteristics
BLEService Bot("3E099910-294F-11E5-93BE-AFD1FE6D1DFD");
BLEIntCharacteristic Speed("3E099911-294F-11E5-93BE-AFD1FE6D1DFD", props);
BLEIntCharacteristic Steer("3E099912-294F-11E5-93BE-AFD1FE6D1DFD", props);
BLEIntCharacteristic Reset   ("3E099913-294F-11E5-93BE-AFD1FE6D1DFD", props);

// Bot stuff                      one           two  batteries
const int gyro_prop = 30; //      30            15     
const float gyro_diff = 1.25; //  2 (40/20)     1.25 (25/20)
const int vari_prop = 2; //       5             2
const int vari_diff = 2; //       5             2
const int disp_prop = 30; //      30?           60    x + 1
const float disp_diff = 1.1; //   1?            1.1   1 + (x / 100)

// RC
  // Speed
int speedVal = 0; // from transmitter
unsigned long move_timePrev = 0; // last time we moved a unit
unsigned int move_delay = 0; // time to wait between each move based on speedVal
unsigned long move_safetyTimer = 0; // safety, for last time we received from transmitter
int moveDir = 0; // to move forwards or backwards
  // Steer
int steering = 0;

bool reset = 0; // not reseting to begin with
...
const byte axis = 0;   // the one that measures leaning back and forth
...
/*********** Bluetooth stuff ***********/
  ...
  blePeripheral.addAttribute(Speed);
  blePeripheral.addAttribute(Steer);
...
/*********** Bluetooth stuff ***********/
  ...
  // if we have a central, and we are connected
    ...
    // put safety timers somewhere
      
    // SPEED
    if (Speed.written()) {
       speedVal = Speed.value();
       move_delay = 1000 / abs(speedVal);
       // do something
    }
    
    // if we are supposed to move forward
    if (speedVal > 0) 
      moveDir = 1;
    // else if we are supposed to move backward
    else // speedVal <= 0
      moveDir = -1;
      
    // move
    if (millis() - move_timePrev > move_delay) {
      // if moveDir is positive, we should move forward.
      // so subtract to make motors think they've moved backward,
      // causing them to move forward.
      // Will do opposite if moveDir negative.
      enco1 -= moveDir;
      enco2 -= moveDir;
      move_timePrev += move_delay;
    }
    ...
    // RESET
    if (Reset.written()) {
      reset = Reset.value(); // so can reset all the time
    }
    if (reset) { // allows for constant resetting
      gyro_disp = 0;
      enco1 = 0;
      enco2 = 0;
    }
