...
bool reset = 0; // not reseting to begin with.  For printing.
...
/*********** Bluetooth stuff ***********/
  ...
  // if we have a central, and we are connected
    ...
    // SPEED
    if (Speed.written()) {
       speedVal = map(Speed.value(), -511, 512, 0, 255);
       motor1_result = motor2_result = speedVal;
    }
    ...
    // RESET
    if (Reset.written()) {
      reset = Reset.value(); // for printing
      gyro_disp = 0;
      enco1 = 0;
      enco2 = 0;
    }
