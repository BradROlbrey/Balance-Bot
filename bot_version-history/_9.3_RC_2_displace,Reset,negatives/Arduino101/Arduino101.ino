...
int gyro_prop = 15; //      30            15     
float gyro_diff = 1.25; //  2 (40/20)     1.25 (25/20)
int vari_prop = 2; //       5             2
int vari_diff = 2; //       5             2
int disp_prop = 60; //      30?           60    (x + 1)
...
/*********** Gyro stuff ***********/
  ...
  float gyro_temp = 0;
  int polls = 1, targetPolls = 300; // just trying to tranfer data atm, not balance
  // reset gyro_timePrev
...
/*********** I2C stuff ***********/
  if (millis() - receive_timePrev > receive_interval) { // 10 times a second
    ...
    if (Wire.available()){       // check if we've received something
      ...
      //Serial.print("converting readings, ");
      for(int i = 0; i < 2; i++) {
        // get the reading
        data[i] = map(dataRx[2*i], 0, 255, 0, 5);
