...
int gyro_diff = 1; // differential of gyro
...
/*********** Gyro stuff ***********/
const int update_rate = 100; // update frequency: x times per second
const int update_time = 1000/update_rate; // ms per update
...
/*********** Gyro stuff ***********/
  CurieIMU.begin();
  CurieIMU.setGyroRate(update_rate);
  ...
  while (polls <= targetPolls) { // number of polls
    if (millis() - gyro_timePrev >= update_time) {
      gyro_temp += CurieIMU.readGyro(axis);
      gyro_timePrev += update_time;
      polls++;
    }
  }
...
/*********** Gyro stuff ***********/
  gyro_read = CurieIMU.readGyro(axis);
  gyro_disp += gyro_read;


/*********** Motor PID stuff ***********/
  // if the bot is not completely tipped over
  if (gyro_read > -60 && gyro_read < 60) {
    gyro_error = gyro_read - gyro_target; // error in tilt from 90 degrees
    //result = error * prop + tilt_change * diff;
    gyro_result = gyro_error * gyro_prop;
    ...
    if (gyro_result > 0) { // target greater than current pos. We are behind target, 
    ...
    else if (gyro_result < 0) { // target less than current pos. We are in front of target,
