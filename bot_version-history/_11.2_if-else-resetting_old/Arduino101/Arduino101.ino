...
byte gyro_buffer = 25; // gyro_limit - gyro_buffer, area inside of gyro_limit to cap tilt within
...
// bot stuff
  if (!data[2]) {  // if we are not resetting, make the bot do bot stuff
    // balance time loop
    if (millis() - gyro_timePrev > updateTime) {
      //loopTime = micros();
...
// reset stuff  
    ...
    enco1 = 0;
    enco2 = 0;
    brakes(); // and turn on the brakes so it stays put
  }  // end resetting
