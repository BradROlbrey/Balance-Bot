...
// gyro moving center by exponential average
float gyro_center = 0;
const float alpha = .9965;       // multiplies with old reading
const float beta = 1 - alpha;  // multiplies with new reading
bool tippedOver = false;      // brakes the bot if it has tipped over
  // is set true if the bot is tipped over and only
  // set false if we are resetting the bot

const byte maxSpeed = 10;  // 7 worked well
const byte maxSteer = 6;   // 3 worked well
...
/*********** I2C stuff ***********/
  if (millis() - receive_timePrev > receive_interval) { // 10 times a second
    ...
    if (Wire.available()){       // check if we've received something
      ...
      data[0] = map(dataRx[0], 0, 255, 0, maxSpeed);
      // see if it was negative
      if (dataRx[1])
        data[0] = -data[0];

      // convert variance (steering left/right)
      data[1] = map(dataRx[2], 0, 255, 0, maxSteer);
      // see if it was negative
      if (dataRx[3])
        data[1] = -data[1];
  ...
  // not-resetting, i.e. balancing, loop
  if (!data[2]) {  // if we are not resetting, make the bot do stuff
    // balance time loop
    if (millis() - gyro_timePrev > updateTime) {
      //loopTime = micros();
...
/*********** Displace stuff ***********/
      ...
      disp_avg = (enco1 + enco2) / 2; // probs compute intensive but whatevs
      ...
      // if the target is less than the limit plus a smaller buffer,
      if (gyro_target < gyro_center - gyro_limit + gyro_buffer) {
        // set it to that
        gyro_target = gyro_center - gyro_limit + gyro_buffer;
      } // else
      // if the target is greater than the limit minus the smaller buffer
      else if (gyro_target > gyro_center + gyro_limit - gyro_buffer) {
        // set it to that
        gyro_target = gyro_center + gyro_limit - gyro_buffer;
      }/**/
...
/*********** Gyro stuff ***********/
      ...
      // gyro center, exponential average
                    // old reading        // new reading
      gyro_center = alpha*(gyro_center) + beta*(gyro_disp);

      // check if the bot is tipped over
      if (gyro_disp < gyro_center - gyro_limit ||  // lower bound
          gyro_disp > gyro_center + gyro_limit     // upper bound
          ) {
        tippedOver = true;  // if so, set true for later
      }
...
/*********** Motor PID stuff ***********/
      ...
      // if the bot is not tipped over, set the motors
      if (!tippedOver) {
  ...
  else {  // data[2] is high, i.e. i'm trying to reset everything
    gyro_disp = gyro_center = 0;
    enco1 = 0;
    enco2 = 0;
    brakes();
    tippedOver = false;  // I'm probably setting the bot back upright
    gyro_timePrev = millis();  // this is insanely important!!!
  }  // end resetting
...
/*********** Gyro stuff ***********
  ...
  /***** gyro center stuff *****
  //Serial.print(gyro_center - gyro_limit);
  //space(gyro_center - gyro_limit);
  //Serial.print(gyro_center - gyro_limit + gyro_buffer);
  //space(gyro_center - gyro_limit + gyro_buffer);
  Serial.print(gyro_center);
  space(gyro_center);
  //Serial.print(gyro_center + gyro_limit - gyro_buffer);
  //space(gyro_center + gyro_limit - gyro_buffer);
  //Serial.print(gyro_center + gyro_limit);
  //space(gyro_center + gyro_limit);
  Serial.print(tippedOver);
  Serial.print('\t');
