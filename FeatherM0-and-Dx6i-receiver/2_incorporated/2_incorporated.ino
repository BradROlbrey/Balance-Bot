...
void loop(){
  if (millis() - calcPrevTime > calcInterval) {
    ...
    if (elevDuration >= 1500 + buff) {
      data[0] = map(elevDuration, 1500+buff, 1920, 0, 255);
      data[1] = 0; // not negative
    }
    else if (elevDuration <= 1500 - buff) {
      // the shorter the high, the more negative (sign is separate) the number should be
      data[0] = map(elevDuration, 1080, 1500-buff, 255, 0);
      data[1] = 1; // negative
    }
    ...
    if (ruddDuration >= 1500 + buff) {
      data[2] = map(ruddDuration, 1500+buff, 1910, 0, 255);
      data[3] = 0; // not negative
    }
    else if (ruddDuration <= 1500 - buff) {
      // the shorter the high, the more negative (sign is separate) the number should be
      data[2] = map(ruddDuration, 1090, 1500-buff, 255, 0);
      data[3] = 1; // negative
    }
    else{ // it is within the buffer
      data[2] = 0;
